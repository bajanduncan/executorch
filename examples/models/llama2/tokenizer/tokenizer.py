...
class Tokenizer:
    def __init__(self, model_path: str):
        assert os.path.isfile(model_path), f"Need a valid tokenizer model path but got {model_path}"
        ...
        logging.info(f"#words: {self.n_words} - BOS ID: {self.bos_id} - EOS ID: {self.eos_id}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--tokenizer-model", type=str, default="tokenizer.model", help="path to tokenizer model, given by sentencepiece"
    )
    parser.add_argument(
        "-o", "--output-path", type=str, default=None, help="output path of postprocessed tokenizer model"
    )
    parser.add_argument(
        "-p",
        "--prepend-padding",
